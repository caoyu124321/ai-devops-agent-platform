...
| Codex 本机登录体验 | 服务启动后，Codex 通过 URL 型 MCP 的 OAuth 认证挑战完成浏览器登录；项目不启动 stdio MCP，也不使用 Windows 凭据管理器。 |
...
- 将本机 stdio MCP 与 Windows 凭据管理器实现移出 Codex 对话运行路径。
- 提供项目级 URL 型 MCP 配置；服务启动后，Codex 直接通过 OAuth 完成注册、登录和登录状态查询。
...
- 当前不新增审计模块或审计持久化要求。
- 不提供 stdio、一次性登录链接或 Windows 凭据管理器回退。
...
### US-MCP-04 在 Codex 对话中完成认证操作

作为已启动本机平台的开发用户，我希望直接在 Codex 对话中注册、登录并查询登录状态，以便不启动终端、不提供密码给对话，也不依赖 Windows 凭据管理器。

**验收标准**：

- AS-MCP-04.1：项目配置只声明 `http://127.0.0.1:8080/mcp` URL 型 MCP 与 OAuth，不声明 stdio 命令。
- AS-MCP-04.2：`register_ai_devops` 返回安全浏览器注册链接，注册资料和密码仅在浏览器页面提交。
- AS-MCP-04.3：`login_ai_devops` 未认证时返回 OAuth 认证挑战；认证成功后返回安全用户摘要，不返回密码或 Token。
- AS-MCP-04.4：`get_ai_devops_login_status` 在认证成功后返回 `LOGGED_IN`、安全用户摘要和会话失效时间，不返回 Token。
- AS-MCP-04.5：默认 H2 本机库在服务启动时初始化 IAM/OAuth 所需表；外部数据库仍使用版本化迁移。
...
- FR-MCP-008：项目级 Codex 配置必须只使用 `http://127.0.0.1:8080/mcp` URL 型 MCP 与 OAuth，不得配置 stdio 命令或 Windows 凭据管理器适配器。
- FR-MCP-009：远程 MCP 必须提供 `login_ai_devops` 登录触发工具。该工具不接收或返回密码、Token 或登录链接；未认证调用必须返回 OAuth 认证挑战，认证成功后仅返回当前登录状态摘要。
- FR-MCP-010：远程 MCP 必须提供 `get_ai_devops_login_status`。该工具认证成功后只返回 `LOGGED_IN`、安全用户摘要和会话失效时间；未认证时返回 OAuth 认证挑战。
- FR-MCP-011：默认 H2 本机数据库必须在服务启动时初始化 IAM/OAuth 所需表；非 H2 数据库不得由应用自动建表，必须执行版本化迁移。
